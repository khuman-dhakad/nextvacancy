...
export const RelatedJobs: React.FC<RelatedJobsProps> = ({
  jobs = [],
  categoryTitle = "Related Active Recruitments",
  className = "",
}) => {
  if (!jobs || jobs.length === 0) return null;

  return (
    <section aria-label="Related Opportunities" className={className}>
            ...
            >
              <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[10px] font-bold uppercase text-slate-500 bg-white border border-slate-200 px-2 py-0.5 rounded truncate max-w-[130px]">
                    {job.organization}
                  </span>
                  <Badge
                  ...
                  >
                    {job.status === "OPEN" ? "Active" : job.status === "ENDING_SOON" ? "Ending" : job.status}
                  </Badge>
                </div>
                ...
                <div className="space-y-1 text-[11px] text-slate-600 font-medium pt-1">
                  ...
                  <div className="flex items-center gap-1.5 text-red-600 font-bold">
                    ...
                    <span>Last Date: {job.importantDates.applicationEndDate || "Check Circular"}</span>
                  </div>
                </div>
              </div>
